...
class RangeSliderH(Frame):
    ...
    def __init__(self, master, variables, Width = 400, Height = 80, min_val = 0, max_val = 1, padX=3,
                    ...
                    bar_color_inner='#5c8a8a', line_s_color="Yellow", bar_color_outer='#c2d6d6', line_color = '#476b6b', bgColor= '#ffffff',
                    show_value = True, digit_precision='.1f', valueSide='TOP', font_family='Times', font_size=1, suffix="", left_color = "Green", right_color = "Red", values = None):
        if values == None:
            diff = max_val - min_val
            values = [min_val+diff/3,min_val+diff/3*2]
        self.LINE_COLOR_L=left_color
        self.LINE_COLOR_R=right_color
        ...
        if self.draw=='auto':
            self.slider_x = 0#RangeSliderH.BAR_RADIUS#+self.padx # x pos of the slider (left side)
            a = 0
        else:
        ...
        self.track[0:2] = self.__addTrackL(self.slider_x, self.slider_y, self.canv_W-self.slider_x, self.slider_y, self.bars[0]["Pos"], self.bars[1]["Pos"])
        self.track[1:3] = self.__addTrackR(self.slider_x, self.slider_y, self.canv_W-self.slider_x, self.slider_y, self.bars[0]["Pos"], self.bars[1]["Pos"])
        tempIdx=0
    ...
    def __addTrackL(self, startx, starty, endx, endy, posL, posR):
        rangeOutL = self.canv.create_line(startx, starty, startx+posL*(endx-startx), endy, fill = self.LINE_COLOR_L, width = RangeSliderH.LINE_WIDTH)
        rangeS = self.canv.create_line(startx+posL*(endx-startx), starty, endx-(1-posR)*(endx-startx), endy, fill = RangeSliderH.LINE_S_COLOR, width = RangeSliderH.LINE_WIDTH)
        return [rangeOutL, rangeS]

    def __addTrackR(self, startx, starty, endx, endy, posL, posR):
        rangeS = self.canv.create_line(startx+posL*(endx-startx), starty, endx-(1-posR)*(endx-startx), endy, fill = RangeSliderH.LINE_S_COLOR, width = RangeSliderH.LINE_WIDTH)
        rangeOutR = self.canv.create_line(endx-(1-posR)*(endx-startx), starty, endx, endy, fill = self.LINE_COLOR_R, width = RangeSliderH.LINE_WIDTH)
